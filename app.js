...
$('btnSalvarConvite').addEventListener('click', async ()=>{
  ...
  if(email === state.user.email.toLowerCase()){
    toast('Você já tem acesso — não é possível convidar seu próprio e-mail.', true); return;
  }
  try{
...
async function saldoAnteriorA(dataLimiteStr){
  const id = state.igrejaAtualId;
  async function soma(tipo){
    const q = query(collection(db,'igrejas',id,'lancamentos'), where('tipo','==',tipo), where('dataStr','<',dataLimiteStr));
    try{
      const snap = await getAggregateFromServer(q, { total: sum('valor') });
      return snap.data().total || 0;
    } catch(e){
  ...
  const [r,d] = await Promise.all([soma('receita'), soma('despesa')]);
  return r - d;
}
...
$('btnPdfMensal').addEventListener('click', async ()=>{
  ...
  const saldoAnterior = await saldoAnteriorA(dataInicioMes);
...
$('btnPdfAnual').addEventListener('click', async ()=>{
  ...
  pdf.autoTable({
    startY: y, head:[['Mês','Receitas','Despesas','Saldo']], body: linhas,
...
$('btnPdfFiel').addEventListener('click', async ()=>{
  ...
  const yFinal = pdf.lastAutoTable.finalY + 10;
