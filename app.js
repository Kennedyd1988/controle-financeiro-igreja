...
async function resumoAnualComSaldo(ano){
  ...
  let acumulado = await saldoAnteriorA(`${ano}-01-01`);
  return porMes.map(m => {
...
async function saldoAnteriorA(dataLimiteStr){
  const id = state.igrejaAtualId;
  const q = query(collection(db,'igrejas',id,'lancamentos'), where('dataStr','<',dataLimiteStr));
  const snaps = await getDocs(q);
